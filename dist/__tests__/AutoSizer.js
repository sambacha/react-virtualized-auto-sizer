...
function DefaultChildComponent({ height, width, foo, bar }) {
    return (_jsx("div", { children: `width:${width}, height:${height}, foo:${foo}, bar:${bar}` }));
}
describe('AutoSizer', () => {
    function getMarkup({ bar = 123, ChildComponent = DefaultChildComponent, className = undefined, defaultHeight = undefined, defaultWidth = undefined, disableHeight = false, disableWidth = false, foo = 456, height = 100, onResize, paddingBottom = 0, paddingLeft = 0, paddingRight = 0, paddingTop = 0, style = undefined, width = 200, } = {}) {
        const wrapperStyle = {
        ...
        return (_jsx("div", Object.assign({ style: wrapperStyle }, { children: _jsx(AutoSizer, Object.assign({ className: className, defaultHeight: defaultHeight, defaultWidth: defaultWidth, disableHeight: disableHeight, disableWidth: disableWidth, onResize: onResize, style: style }, { children: ({ height, width }) => (_jsx(ChildComponent, { width: disableWidth ? undefined : width, height: disableHeight ? undefined : height, bar: bar, foo: foo })) })) })));
    }
    ...
    function simulateResize({ element, height, width }) {
    ...
    it('should update :height after a resize event', (done) => __awaiter(void 0, void 0, void 0, function* () {
        const rendered = findDOMNode(render(getMarkup({
            height: 100,
            width: 200,
        })));
        expect(rendered.textContent).toContain('height:100');
        expect(rendered.textContent).toContain('width:200');
        yield simulateResize({ element: rendered, height: 400, width: 300 });
    ...
    describe('onResize and (re)render', () => {
        it('should trigger when size changes', (done) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        it('should only trigger when height changes for disableWidth == true', (done) => __awaiter(void 0, void 0, void 0, function* () {
            const onResize = jest.fn();
            const ChildComponent = jest
                .fn()
                .mockImplementation(DefaultChildComponent);
            ...
            ChildComponent.mockClear();
            expect(onResize).toHaveBeenCalledTimes(1);
            yield simulateResize({ element: rendered, height: 100, width: 300 });
            expect(ChildComponent).toHaveBeenCalledTimes(0);
            expect(onResize).toHaveBeenCalledTimes(1);
            yield simulateResize({ element: rendered, height: 200, width: 300 });
            expect(ChildComponent).toHaveBeenCalledTimes(1);
            expect(onResize).toHaveBeenCalledTimes(2);
            done();
        }));
        it('should only trigger when width changes for disableHeight == true', (done) => __awaiter(void 0, void 0, void 0, function* () {
            const onResize = jest.fn();
            const ChildComponent = jest
                .fn()
                .mockImplementation(DefaultChildComponent);
            ...
            ChildComponent.mockClear();
            expect(onResize).toHaveBeenCalledTimes(1);
            yield simulateResize({ element: rendered, height: 200, width: 200 });
            expect(ChildComponent).toHaveBeenCalledTimes(0);
            expect(onResize).toHaveBeenCalledTimes(1);
            yield simulateResize({ element: rendered, height: 200, width: 300 });
            expect(ChildComponent).toHaveBeenCalledTimes(1);
